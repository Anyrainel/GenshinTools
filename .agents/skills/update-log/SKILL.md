...
- Ongoing maintenance of fragile third-party integrations, such as "修复米游社账号数据导入" / "Fixed Miyoushe account imports". These integrations can break again quickly as upstream services change, so keeping them working is a continuous effort, not a durable fix to announce. Omit these maintenance changes rather than rephrasing them as reliability improvements.
...
- For included internal/technical changes (sync, storage), describe the **outcome**, not the mechanism — e.g. "Saved teams now keep their selected attack counts after importing", not "updated the persisted formula-count schema". Don't enumerate implementation specifics players can't act on. Staying high-level also keeps you accurate: don't assert mechanism details inferred from a diff. This tone guidance does not override the exclusions above.
