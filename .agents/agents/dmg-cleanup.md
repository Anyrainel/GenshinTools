...
```
Read the output file `scripts/data/<id>.txt`. This contains game text (ZH) and param templates. The last line shows the implementation location (e.g., `IMPL: character4Mondstadt.ts L713–L787`). Use the Read tool with the file path (`src/lib/dmgcalc/impl/<filename>`) and the line range to read the implementation code.
