...
export function ComputeView({ onJumpToCharacter }: ComputeViewProps) {
  ...
  const characterToBuildIds = useBuildsStore((state) => state.characterToBuildIds);
  ...
  const filteredSets = useMemo(() => {
    return artifacts.filter(set => {
      if (searchQuery && !t.artifact(set.id).toLowerCase().includes(searchQuery.toLowerCase())) return false;
  ...
  const handleComputeOptionChange = useCallback(<K extends keyof ComputeOptions>(key: K, value: ComputeOptions[K]) => {
  ...
  return (
    <div ref={containerRef} className="flex flex-row gap-4 lg:gap-6 h-full pt-4">
      ...
      <div className="flex-1 min-w-0 flex flex-col h-full">
        ...
        <div ref={mainScrollRef} className="flex-1 overflow-y-auto" style={{ scrollBehavior: 'auto' }}>
