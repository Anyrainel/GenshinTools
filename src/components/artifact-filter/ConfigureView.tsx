...
export const ConfigureView = forwardRef<ConfigureViewRef>((props, ref) => {
  ...
  const filteredAndSortedCharacters = (() => {
    const filtered = characters.filter(character => {
      ...
      if (filters.rarities.length > 0 && !filters.rarities.includes(character.rarity)) {
        return false;
      }
      return true;
    });
  ...
  const hasActiveFilters = filters.elements.length > 0 ||
  ...
  return (
    <>
      <div ref={containerRef} className="flex flex-col gap-4 lg:flex-row lg:gap-6 h-full pt-4">
        ...
        <section className="flex-1 min-w-0 flex flex-col h-full overflow-hidden">
            ...
            >
              ...
              {hasActiveFilters && (
                <span className="ml-1 px-1.5 py-0.5 text-xs bg-primary text-primary-foreground rounded-full">
                  {[filters.elements, filters.weaponTypes, filters.regions, filters.rarities]
                    .flat().length}
                </span>
              )}
            </Button>
          ...
          {filteredAndSortedCharacters.length === 0 ? (
            <div ref={mainScrollRef} className="flex-1 overflow-y-auto overflow-hidden" style={{ scrollBehavior: 'auto' }}>
...
const VirtualizedCharacterList = forwardRef<ConfigureViewRef, CharacterListProps>(({ characters, scrollRef }, ref) => {
  ...
  return (
    <div ref={scrollRef} className="flex-1 overflow-y-auto overflow-hidden" style={{ scrollBehavior: 'auto' }}>
      <div
        className="relative"
        style={{
