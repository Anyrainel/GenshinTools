...
export function WeaponChoiceView() {
  ...
  const activeWeaponChoiceTeamId = useSessionNavStore(
    (s) => s.activeWeaponChoiceTeamId
  );
  const setActiveWeaponChoiceTeamId = useSessionNavStore(
  ...
  const weaponChoiceLabel = t.ui("teamComp.tabWeaponChoice");
  ...
  if (activeWeaponChoiceTeamId) {
    const team = teams.find((tm) => tm.id === activeWeaponChoiceTeamId);
    if (!team) {
      setTimeout(() => setActiveWeaponChoiceTeamId(null), 0);
      return null;
    }
            ...
            return (
              ...
              >
                <TeamCard
                  ...
                  onSelect={() => setActiveWeaponChoiceTeamId(team.id)}
                  selectLabel={weaponChoiceLabel}
                  selectIcon={Sword}
