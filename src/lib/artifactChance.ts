...
export function computeSlotChance(
  ...
  if (slot === "flowerPlume") {
    return (
      (computeSubstatSuccessProbability("hp", slotConfig) +
        computeSubstatSuccessProbability("atk", slotConfig)) /
      2
    );
  }
  ...
  const effectiveChoices =
    choices.length > 0
      ? choices
      : Object.keys(mainStatPool).map((stat) => ({
  ...
  for (const choice of effectiveChoices) {
    ...
    probability +=
      mainProb *
      computeSubstatSuccessProbability(choice.stat as MainStatPlus, slotConfig);
  }
...
function subsetProbability(
  ...
  for (let i = 0; i < weights.length; i++) {
    ...
    value +=
      (weight / denominator) *
      subsetProbability(maskWithout, weights, totalWeight, memo);
  }
