...
export const elementalMainStats: MainStat[] = ['pyro%', 'hydro%', 'anemo%', 'electro%', 'dendro%', 'cryo%', 'geo%', 'phys%'];

export const statPools = {
  flower: ['hp'] as const,
  plume: ['atk'] as const,
  sands: ['atk%', 'hp%', 'def%', 'em', 'er'] as const,
  goblet: ['atk%', 'hp%', 'def%', 'em', 'pyro%', 'hydro%', 'anemo%', 'electro%', 'dendro%', 'cryo%', 'geo%', 'phys%'] as const,
  circlet: ['cr', 'cd', 'atk%', 'hp%', 'def%', 'em', 'heal%'] as const,
  substat: ['cr', 'cd', 'atk%', 'hp%', 'def%', 'em', 'er', 'atk', 'hp', 'def'] as const
};
...
export const statPoolWithWeights = {
  ...
  goblet: { 'atk%': 21.25, 'hp%': 21.25, 'def%': 20, 'em': 2.5, 'pyro%': 5, 'hydro%': 5, 'anemo%': 5, 'electro%': 5, 'dendro%': 5, 'cryo%': 5, 'geo%': 5, 'phys%': 5 },
  circlet: { 'cr': 10, 'cd': 10, 'atk%': 22, 'hp%': 22, 'def%': 22, 'em': 4, 'heal%': 10 },
  substat: { 'cr': 7.5, 'cd': 7.5, 'atk%': 10, 'hp%': 10, 'def%': 10, 'em': 10, 'er': 10, 'atk': 15, 'hp': 15, 'def': 15 }
}
...
const createRecord = <Item, Key extends PropertyKey>(
  ...
  return items.reduce<Record<Key, Item>>((acc, item) => {
    acc[getKey(item)] = item;
    return acc;
  }, {} as Record<Key, Item>);
};

const freezeRecord = <MapType extends Record<PropertyKey, unknown>>(record: MapType) =>
  Object.freeze(record) as Readonly<MapType>;

export const charactersById = freezeRecord(
  createRecord<Character, Character['id']>(characters, (character) => character.id)
);
...
export const artifactHalfSetsById = freezeRecord(
  createRecord<ArtifactHalfSet, ArtifactHalfSet['id']>(artifactHalfSets, (halfSet) => halfSet.id)
);

export const elementResourcesByName = freezeRecord(
  createRecord<ElementResource, ElementResource['name']>(elementResources, (element) => element.name)
);
