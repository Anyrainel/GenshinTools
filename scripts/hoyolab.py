import re
import time
from typing import TypedDict, List, Optional, Tuple, Set
...
def extract_character_from_card(card: Locator, index: int) -> Optional[ScrapedCharacter]:
    try:
    ...
    try:
        ...
        if not original_image_url or is_placeholder_image(original_image_url):
            print(f"ERROR: {name} has placeholder image: {original_image_url if original_image_url else 'None'}")
            return None
...
def scrape_characters(page: Page, language: str = "en") -> List[ScrapedCharacter]:
    ...
    for i, card in enumerate(character_cards):
        ...
        if char_data:
            ...
            print(f"Character {i+1}: {char_data['name']} ({char_data['element']}, {char_data['rarity']}*)")
...
def scrape_weapons(page: Page, language: str = "en") -> List[ScrapedWeapon]:
    ...
    # The wrapper class found in inspection is .genshin-show-weapon-item
    ...
    for i, card in enumerate(weapon_cards):
        ...
        if weapon_data:
            ...
            print(f"Weapon {i+1}: {weapon_data['name']} ({weapon_data['rarity']}*)")
            
    return weapons

def scrape_elements_and_weapons(page: Page, language: str = "en") -> Tuple[List[ResourceOutput], List[ResourceOutput]]:
