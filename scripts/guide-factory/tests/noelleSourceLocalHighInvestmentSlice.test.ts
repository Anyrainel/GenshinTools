...
import { formatNoelleSourceLocalHighInvestmentSliceSummary } from "../src/assemble-noelle-source-local-high-investment-slice";
...
const HOLDOUTS = [
  ...
  [
    `kqm:character_guide:${SOURCE_GUIDE_ID}:recommendation.mainStats.circlet[1].conditions`,
    "7444217b95d562ae6cffb22b51e7c6ef6da6891851967415925d6713c254e2c6",
  ],
  [
    `kqm:character_guide:${SOURCE_GUIDE_ID}:recommendation.mainStats.goblet[1].conditions`,
    "778b1c674223e02b4b58c3903c6ac809f5bab52e2c216e93003320fd1657f748",
  ],
  ...[0, 1, 2].map((index) => [
    `kqm:character_guide:${SOURCE_GUIDE_ID}:recommendation.substats[${index}].conditions`,
...
describe("Noelle source-local high-investment slice", () => {
  it("authenticates the exact 3+12+1 source corpus and C6 request projection", async () => {
    ...
    expect(formatNoelleSourceLocalHighInvestmentSliceSummary(report)).toContain(
  ...
  it("pins every selected literal, payload, source predicate, and numeric request binding", async () => {
    const report = buildNoelleSourceLocalHighInvestmentSliceReport(await fixture());
  ...
  it("preserves three unresolved source cells and resolves only through scoped C6 request facts", async () => {
    const report = buildNoelleSourceLocalHighInvestmentSliceReport(await fixture());
    const slice = requiredSlice(report);
  ...
  it("fails closed on snapshot, payload, team, registry, and byte/object drift", async () => {
    ...
    expect(
      buildNoelleSourceLocalHighInvestmentSliceReport(snapshotInput)
        .comparisonStatus,
    ).toBe("not-comparable");
    ...
    expect(
      buildNoelleSourceLocalHighInvestmentSliceReport(payloadInput)
        .comparisonStatus,
    ).toBe("not-comparable");
    ...
    expect(
      buildNoelleSourceLocalHighInvestmentSliceReport(teamInput).comparisonStatus,
    ).toBe("not-comparable");
    ...
    expect(
      buildNoelleSourceLocalHighInvestmentSliceReport(registryInput)
        .comparisonStatus,
    ).toBe("not-comparable");
    ...
    (objectInput.manualSnapshotInput as Record<string, unknown>).capturedAt =
      "2099-01-01";
    expect(
      buildNoelleSourceLocalHighInvestmentSliceReport(objectInput).issues[0]
        ?.message,
  ...
  it("authenticates the durable report and rejects report or path tampering", async () => {
    const input = await fixture();
    const canonical = buildNoelleSourceLocalHighInvestmentSliceReport(input);
    ...
    expect(
      authenticateNoelleSourceLocalHighInvestmentSliceReport(durable, input),
    ).toMatchObject({ authenticated: true });

    const changed = structuredClone(durable);
    changed.summary.effectiveMatchedCount += 1;
    expect(
      authenticateNoelleSourceLocalHighInvestmentSliceReport(changed, input),
    ).toMatchObject({
      authenticated: false,
      reason: "serialized-report-mismatch",
    });
    expect(() =>
      requireComparableNoelleSourceLocalHighInvestmentSliceReport(changed, input),
    ...
    expect(
      buildNoelleSourceLocalHighInvestmentSliceReport(missingPathInput).issues[0]
        ?.message,
    ).toContain("generatedFrom path closure drifted");
  });
});

async function fixture(): Promise<BuildNoelleSourceLocalHighInvestmentSliceInput> {
  const [
    repositoryInput,
    manualSnapshotInput,
    manualIndexInput,
...
): NonNullable<NoelleSourceLocalHighInvestmentSliceReport["sourceLocalSlice"]> {
