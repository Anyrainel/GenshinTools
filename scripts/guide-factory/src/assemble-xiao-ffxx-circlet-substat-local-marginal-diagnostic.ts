...
import { XIAO_FFXX_FIVE_STAR_SOURCE_GROUP_VALIDATION_DIAGNOSTIC_REPORT_PATH } from "./xiaoFfxxFiveStarSourceGroupValidationDiagnostic";
...
export function formatXiaoFfxxCircletSubstatLocalMarginalDiagnosticSummary(
  report: XiaoFfxxCircletSubstatLocalMarginalDiagnosticReport,
): string {
  return (
    `Wrote ${report.summary.reconstructionControlCount} Xiao FFXX reconstruction controls and ` +
    `${report.summary.latticeNodeCount} experimental Circlet/substat local-marginal nodes ` +
    `from ${report.summary.replayCount} dual-path-agreeing replays; no source guard, selection, rank, winner, or recommendation resolved.`
  );
}

export async function runXiaoFfxxCircletSubstatLocalMarginalDiagnosticCli(): Promise<void> {
  const [
    repositoryInput,
    ...
    readJson(XIAO_FFXX_FIVE_STAR_SOURCE_GROUP_VALIDATION_DIAGNOSTIC_REPORT_PATH),
    Promise.all(
  ...
  if (!authentication.authenticated) {
    throw new Error(
      `Fresh Xiao FFXX Circlet/substat local-marginal diagnostic failed self-authentication (${authentication.reason}).`,
    );
  }
  await writeJson(
    XIAO_FFXX_CIRCLET_SUBSTAT_LOCAL_MARGINAL_DIAGNOSTIC_REPORT_PATH,
  ...
  console.log(
    formatXiaoFfxxCircletSubstatLocalMarginalDiagnosticSummary(report),
  );
}
...
if (
  ...
  await runXiaoFfxxCircletSubstatLocalMarginalDiagnosticCli();
}
