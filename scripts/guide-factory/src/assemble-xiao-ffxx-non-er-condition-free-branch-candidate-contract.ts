...
export function formatXiaoFfxxNonErConditionFreeBranchCandidateSummary(
  report: XiaoFfxxNonErConditionFreeBranchCandidateContractReport,
): string {
  if (report.comparisonStatus !== "comparable") {
    return `Xiao FFXX non-ER branch candidate contract is not comparable: ${report.issues.length} issue(s).`;
  }
  return (
    `Wrote ${report.summary.partialCandidateCount} partial Xiao FFXX non-ER candidates ` +
    `across ${report.summary.branchGroupCount} source groups with ` +
    `${report.summary.provenanceBindingCount} provenance bindings; ` +
    `complete candidates/builds: 0/0.`
  );
}

export async function runXiaoFfxxNonErConditionFreeBranchCandidateCli(): Promise<void> {
  const [
    repositoryInput,
    manualSnapshotInput,
    manualIndexInput,
  ...
  console.log(formatXiaoFfxxNonErConditionFreeBranchCandidateSummary(report));
}
...
if (
  ...
  await runXiaoFfxxNonErConditionFreeBranchCandidateCli();
}
