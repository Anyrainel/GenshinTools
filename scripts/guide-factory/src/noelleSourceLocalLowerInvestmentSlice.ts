...
const GUIDE_DEFINITION = {
  ...
  sourceRecordId: "noelle-c0-c5-talent-9-artifact-stats-luna-viii",
...
const SELECTED_OCCURRENCES = [
  {
    ...
    exactSourceItem: {
      statIds: ["atk%"],
      conditions: ["Noelle is C0–C5 and her Burst Talent is Level 9."],
    },
...
const CAUTIONS = [
  "The selected occurrences remain agent-assisted and unreviewed source observations.",
  "The source predicate remains unresolved; only the request overlay evaluates the explicit Noelle constellation and Burst Talent facts together.",
  "The selected lower-investment main-stat cells are preserved independently and are not assembled into a build.",
...
export interface NoelleSourceLocalLowerInvestmentSliceSourceFile {
  path: string;
  text: string;
}

export interface BuildNoelleSourceLocalLowerInvestmentSliceInput {
  repositoryInput: unknown;
  manualSnapshotInput: unknown;
  manualIndexInput: unknown;
  sourceRegistryInput: unknown;
  sourceFiles: readonly NoelleSourceLocalLowerInvestmentSliceSourceFile[];
  generatedFrom: readonly GeneratedFromEntry[];
}

export interface NoelleSourceLocalLowerInvestmentHoldoutOccurrence {
  occurrenceId: string;
...
export interface NoelleSourceLocalLowerInvestmentSelectedOccurrence {
  occurrenceId: string;
  characterId: "noelle";
  sourceRecordId: string;
  repositoryRecordId: string;
...
export interface NoelleSourceLocalLowerInvestmentEmptyOccurrence {
  occurrenceId: string;
  sourceRecordId: string;
  repositoryRecordId: string;
  manualClaimPath: string;
...
export interface NoelleSourceLocalLowerInvestmentSliceReport {
  schemaVersion: 1;
  reportType: "noelle-source-local-lower-investment-slice";
  ...
  selectedOccurrences: NoelleSourceLocalLowerInvestmentSelectedOccurrence[];
  holdoutOccurrences: NoelleSourceLocalLowerInvestmentHoldoutOccurrence[];
  emptyOccurrences: NoelleSourceLocalLowerInvestmentEmptyOccurrence[];
...
export type NoelleSourceLocalLowerInvestmentSliceAuthentication =
  | {
      authenticated: true;
      canonicalReport: NoelleSourceLocalLowerInvestmentSliceReport;
    }
  | {
      authenticated: false;
...
export const NOELLE_SOURCE_LOCAL_LOWER_INVESTMENT_SLICE_SOURCE_FILE_PATHS = [
...
export const NOELLE_SOURCE_LOCAL_LOWER_INVESTMENT_SLICE_INPUT_PATHS = [
  ...new Set([
    "scripts/guide-factory/src/noelleSourceLocalLowerInvestmentSlice.ts",
    "scripts/guide-factory/src/assemble-noelle-source-local-lower-investment-slice.ts",
...
export const NOELLE_SOURCE_LOCAL_LOWER_INVESTMENT_SLICE_REPORT_PATH = path.join(
  FACTORY_ROOT,
  "reports",
  "noelle-source-local-lower-investment-slice.json",
);
...
export function buildNoelleSourceLocalLowerInvestmentSliceReport(
  ...
  try {
    ...
    const selectedOccurrences: NoelleSourceLocalLowerInvestmentSelectedOccurrence[] =
      [];
    ...
    const authenticatedEmptyOccurrences: NoelleSourceLocalLowerInvestmentEmptyOccurrence[] =
    ...
    if (sourceLocalSlice.comparisonStatus !== "comparable") {
      throw new Error(
        `The canonical Noelle lower-investment slice is not comparable: ${sourceLocalSlice.issues
          .map(({ code }) => code)
          .join(", ")}.`,
      );
    }
    ...
    return {
      ...
      rawInputBoundary: {
        ...
        sourceFileCount:
          NOELLE_SOURCE_LOCAL_LOWER_INVESTMENT_SLICE_SOURCE_FILE_PATHS.length,
...
export function authenticateNoelleSourceLocalLowerInvestmentSliceReport(
  serializedReport: NoelleSourceLocalLowerInvestmentSliceReport,
  input: BuildNoelleSourceLocalLowerInvestmentSliceInput,
): NoelleSourceLocalLowerInvestmentSliceAuthentication {
  const canonicalReport = buildNoelleSourceLocalLowerInvestmentSliceReport(input);
  ...
  if (stableJson(serializedReport) !== stableJson(canonicalReport)) {
    return {
      ...
      issues: [
        {
          code: "noelle-source-local-lower-investment.serialized-report-mismatch",
          path: "serializedReport",
          message:
            "Serialized Noelle source-local lower-investment report does not match a fresh canonical rebuild from current exact inputs.",
        },
      ],
    };
  }
...
export function requireComparableNoelleSourceLocalLowerInvestmentSliceReport(
  report: NoelleSourceLocalLowerInvestmentSliceReport,
  input: BuildNoelleSourceLocalLowerInvestmentSliceInput,
): void {
  const authentication = authenticateNoelleSourceLocalLowerInvestmentSliceReport(
    report,
    input,
  );
  if (authentication.authenticated) return;
  throw new Error(
    `Refusing an unauthenticated Noelle source-local lower-investment report (${authentication.reason}): ${authentication.issues
...
function authenticateExpectedSliceSemantics(
  ...
  const safe =
    ...
    report.requestContextReport != null &&
    stableJson(report.requestContextReport.context) ===
      stableJson({
        requestFactsByTeamRecordId: {
          [TEAM_DEFINITION.repositoryRecordId]: {
            characterFactsById: {
    ...
    report.requestContextReport.factProvenance.constellation === "request" &&
    report.requestContextReport.factProvenance.talentLevels === "request" &&
    report.requestContextReport.teamProjections.length === 1 &&
    report.requestContextReport?.teamProjections.every(({ claimProjections }) =>
      claimProjections.length === 3 &&
      claimProjections.every(({ requestContextBindings }) => {
        ...
        const talentRow = binding?.predicateRows[1];
        return (
          ...
          stableJson(binding.requestPredicate) ===
            stableJson(NUMERIC_REQUEST_PREDICATE) &&
          ...
          talentRow?.predicatePath === "requestPredicate.predicates[1]" &&
          talentRow.predicateType === "talent-level-is" &&
          talentRow?.result === "true" &&
          talentRow.factProvenance === "request" &&
  ...
  if (!safe) {
    throw new Error(
      "Noelle source-local lower-investment slice crossed its expected semantic boundary.",
    );
  }
}
...
function authenticateGeneratedFrom(
  ...
  const expectedPaths = [...NOELLE_SOURCE_LOCAL_LOWER_INVESTMENT_SLICE_INPUT_PATHS];
  ...
  if (
    ...
    throw new Error(
      "Noelle source-local lower-investment generatedFrom path closure drifted.",
    );
  }
  if (
    ...
    throw new Error(
      "Noelle source-local lower-investment generatedFrom hashes or paths are invalid.",
    );
  }
  return canonical;
}
...
function payloadForOccurrence(
  ...
  throw new Error(
    `Unsupported Noelle source-local lower-investment claim axis ${occurrence.claimAxis}.`,
  );
}
...
function summaryFromSlice(
  ...
  totalConditionArrayCount: number,
  emptyConditionArrayCount: number,
  holdouts: readonly NoelleSourceLocalLowerInvestmentHoldoutOccurrence[],
): NoelleSourceLocalLowerInvestmentSliceReport["summary"] {
  return {
...
function failedReport(
  ...
  return {
    ...
    issues: [
      {
        code: "noelle-source-local-lower-investment.canonical-input-preparation-failed",
        path: "input",
        message,
      },
    ],
