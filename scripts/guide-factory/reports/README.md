...
- `xiao-ffxx-applicable-claim-projection-contract.json` fresh-authenticates the
  Xiao slice from seventeen exact byte-authenticated declared input files; its
  five JSON inputs also require parsed-object parity.
  Its source-only FFXX view preserves two source matches and one unresolved C6
  occurrence. Its exact-request view adds only one wrapper-owned FFXX/Xiao-C6
  fact, matches all three occurrences, and groups the two identical Anemo
  Goblet occurrences into one payload identity while retaining both provenance
  chains. Payload multiplicity establishes no corroboration or rank. The
...
- `xiao-non-er-equipment-branch-source-slice.json` byte-authenticates eighteen
  exact declared inputs, requires parsed-object parity for five JSON inputs,
  and fresh-authenticates checkpoint 42 through its complete eleven-path
  boundary. It preserves thirteen source recommendation observations: three
  ranked five-star tied groups, six unranked four-star options, ATK% Sands, a
  guarded CR/CD Circlet group, and two guarded offensive-tail substat rows.
  Four observations are source-condition-free and nine remain guarded across
  eight unique condition arrays. It defines no cross-rarity order, selects no
  weapon or Circlet, and emits zero Guide Factory recommendations, candidates,
...
- `xiao-ffxx-unit-expanded-execution-gate.json` authenticates the exact
  121-file raw-byte closure formed by checkpoint 47's complete 118 inputs, the
  durable checkpoint-47 report, and the checkpoint-48 core and CLI. Thirteen
  JSON inputs require byte/object parity before checkpoint 47 is freshly
  rebuilt. A bounded Xiao-specific policy independently expands the two counted
  lines into thirteen contiguous unit lines and requires exact equality with
  the authenticated upstream plan. All six technical-ID-sorted observations
  preserve grouped interpreted damage, pass unit-expanded interpreted/compiled
  agreement, and retain Xianyun's eight-active/three-inactive activation trace.
  The two ranked five-star groups and unranked Deathmatch group remain three
  provenance-only groups with six membership edges. The gate makes zero
  additional calculator replay calls and executes zero comparisons, pairwise
  deltas, damage sorts, ranks, winners, or selections. It supports no guide,
...
- `noelle-hexerei-weapon-team-source-binding.json` authenticates an exact
  16-text-file/hash closure and five JSON byte/object pairs, fresh-authenticates
  the Noelle high-investment slice, and requires its conditional Gest occurrence
  to remain an upstream holdout. It verifies exact normalized consolidation of
  the Gest observation and the Noelle/Durin/Nicole/Xilonen Hexerei example, then
  creates one Guide Factory-authored exact condition-and-heading allowlist
  binding as a validation target. The team member row remains empty, the
  consolidated weapon remains null, and refinement and quantitative performance
  remain missing. The report produces zero candidates, builds, assignments,
