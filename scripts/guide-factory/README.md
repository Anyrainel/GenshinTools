...
194. Pass Guide Factory TypeScript, 65 test files with 554 tests, validation
    ...
    dependency-boundary validation. At checkpoint 36, leave 36-node
    materialization/preflight as the next unimplemented non-ER gate.
195. Add a generic fail-closed lattice-to-runtime preflight that revalidates a
    complete authenticated lattice and materializes each node through a fresh
    existing-runtime `TeamBuild` without invoking a generator or replay.
196. Authenticate the 38-path declared non-self checkpoint input set containing
    selected upstream/runtime dependencies for the checkpoint 36
    Keqing/Ineffa/Furina/Xilonen lattice and exact 11-line Keqing/Ineffa source-
    rotation formula draft before runtime bootstrap. Exclude the producer, CLI,
    and report to avoid self-reference; explicitly do not claim an exhaustive
    or transitive module graph.
197. Resolve all 14 active occurrences exactly, binding source payload hashes
    to selected weapons/refinements or four-piece artifact sets, and reject
    omissions, extras, duplicates, drift, missing registry entities, non-four-
    piece artifacts, ER fields, and unknown input fields.
198. Supply explicit wrapper-owned level-90, C0, talents-10/10/10,
    enemy-110/resistance-0.1, roll-0.85/`8_6`, empty-options/null-aura/no-extra-
    buffs assumptions with all four carry IDs; do not treat them as source
    facts.
199. Materialize 36/36 fresh `TeamBuild` instances and complete 396 objective-
    formula plus 180 non-null unresolved-reference availability checks while
    retaining serialized configs rather than runtime objects.
200. Retain the formula draft's exact eight source-readiness blockers: one
    translation-unreviewed, one partial-token-mapping, five unresolved-formula-
    mapping, and one unresolved-source-token. Mark the result materialized but
    evaluator-not-ready.
201. Execute zero generator, replay, damage-evaluation, scoring, ranking,
    recommendation, optimization, and ER calls. Preserve wrapper-established
    source binding while recording that the generic core establishes none.
202. Add durable report 34 and globally integrate it as report 33. Its byte
    SHA-256 is
    `df5f8d938223063d82cee70cbaf57dd666d10b01b95743e3d84f9e433b9698ec`
    and its size is 555,200 bytes.
203. Pass the focused generic-core/source-wrapper/pipeline suite with 3 files
    and 53 tests, then pass Guide Factory TypeScript, the 67-file/598-test CP37
    baseline, validation with 0 errors and 12 existing warnings, application
    TypeScript, and dependency-boundary validation. Leave bounded generator/
...
228. Add durable report 38 and globally integrate it as report 37. Its current
     ...
     no guide, recommendation, rank, scalar-weight, damage, gameplay, global-
     optimality, promotion, or ER capability is enabled.
229. Authenticate exactly three Xiao condition occurrences against the Version
     5.5 guide snapshot and exact FFXX roster: two Xianyun-roster predicates
     and one independently supplied Xiao-C6 request predicate.
230. Preserve fourteen other nonempty Xiao conditions and four empty arrays as
     unconsumed holdouts; do not inherit predicates from repeated text, hashes,
     nearby recommendations, or rotation prose.
231. Admit only those three occurrences into the generic current binding
     catalog, raising it to 66 = 63 typed + 3 acknowledged and leaving the 15
     ER-deferred display rows unchanged.
232. Translate only the source fixture's `E` and `HP` tokens through an
     explicitly Guide Factory-authored, unreviewed alias table, then compare
     counts without evaluating damage or choosing a winner.
233. Preserve the Xiao Skill 2-versus-2 match and High Plunge 12-versus-11
     mismatch as validation targets. Do not treat count parity as formula-
     semantics proof or the mismatch as a calculator/source correctness verdict.
234. Expand derived formula coverage to three fixtures, twelve observations,
     nine characters, 33 positive rows, 18 zero rows, and two Xiao parity rows,
     while every guide, source-validation, damage, recommendation, and ER
     capability remains false.
235. Require the standalone aggregate to rebuild Xiao's raw fixture/preset
...
241. Byte-authenticate twenty exact declared inputs and fresh-authenticate the
     checkpoint-43 applicable-claim projection before constructing any partial
     candidate.
242. Join only the authenticated Marechaussee Hunter and Anemo Goblet singleton
     payloads into one Guide Factory-authored partial technical candidate; do
     not attribute the whole composition to KQM.
243. Bind the source-only and exact FFXX/Xiao-C6 evidence views to the same
     candidate identity. Preserve the C6 source-unresolved/request-resolved
     lineage and deny any second candidate, vote, corroboration, or rank.
244. Use an explicit six-axis completeness ledger: artifact set and Goblet are
     present; weapon, Sands, Circlet, and substats are missing-not-admitted with
     no defaults. ER remains excluded and deferred outside the denominator.
245. Recompute technical-combination, candidate-identity, upstream-view,
     occurrence-evidence, and view-binding hashes; fail closed with zero output
     on any raw, semantic, provenance, axis, count, or capability drift.
246. Integrate the result as global report 41 with one partial candidate, two
...
them to calculator formulas or attaching them to the team. Checkpoint 42 keeps
that source boundary intact: a separate unreviewed Guide Factory alias table
compares `E = 2` and `HP = 12` with calculator defaults `2` and `11`. The exact
FFXX roster also resolves two Xianyun-conditioned guide occurrences, while an
explicit request overlay resolves one Xiao-C6 occurrence; fourteen nonempty
...
Checkpoint 44 joins those two payload axes into one Guide Factory-authored
partial technical candidate. Source-only and explicit-C6 are separate evidence
...
choice search coverage, the two legacy formula-count drafts, the Xiao count-
parity witness, the derived formula-
...
Checkpoint 37 adds durable report 34 and integrates it as global report 33. Its
wrapper authenticates the declared non-self checkpoint input set containing
38 selected upstream/runtime dependency paths for the lattice and exact formula
draft. It excludes its producer, CLI, and report to avoid self-reference and
explicitly makes no exhaustive or transitive module-graph claim. It resolves
14 active occurrences and materializes 36/36 fresh `TeamBuild` instances under
explicit wrapper-owned assumptions. It completes 396 objective-formula and 180
unresolved-reference availability checks while retaining the exact eight
upstream readiness blockers. Verification passes Guide Factory
...
The separate derived-fixture inventory authenticates three technical reports:
the two legacy formula-plan drafts plus the Xiao count-parity-only witness. Its
standalone path independently rebuilds the Xiao raw fixture/preset semantic
scope before accepting the checked-in witness. It records twelve calculator-
team observations across nine unique characters.
Furina occurs in all three scenarios, Xilonen occurs in two, and the Xiao
fixture contributes Xiao, Xianyun, Furina, and Faruzan as four calculator-team-
only observations because its source fixture supplies no team. All twelve use
exact local C0 calculator assumptions; source investment is unspecified for
the eight legacy observations and absent for the four Xiao observations. The
fixtures contain 33 positive and 18 zero-default formula rows. The Xiao witness
adds two count comparisons: one match and one preserved 12-versus-11 mismatch.
There are still zero source-validated or guide-ready plans. Keqing remains
blocked by eight readiness issues; the older Furina fixture has no comparable
readiness assessment; and the Xiao aliases remain unreviewed. The inventory
performs no formula execution, replay, optimization, damage, or ER work and
supports no recommendation, rank, guide, or source-validation claim.
...
Checkpoint 42 adds the separate Xiao source-local slice. The exact FFXX roster
matches the Marechaussee Hunter and Anemo Goblet occurrences that require
Xianyun. A wrapper-owned Xiao-C6 request fact resolves one second Anemo Goblet
...
Checkpoint 44 fresh-authenticates that projection across a twenty-file byte
closure, then performs one deterministic singleton-axis join. Its six-axis
...
114 unique strings. Across the nonempty arrays, 63 are typed-bound, 3 are exact-
text acknowledged, and 77 are unbound. Excluding three structural ER arrays
leaves 140 non-structural rows: 63 typed, 3 acknowledged, and 74 unbound. The
102 unique non-structural arrays split into 37 typed-only, 64 unbound-only, and
...
Diona/Furina rows; 60 rows are explicitly not energy-deferred, 68 nonempty rows
...
Sucrose but unbound for Sayu and Xianyun. Manual coverage authenticates nine
wrapper families across 20 source files and 81 generated-from paths. The
validator rebuilds the 66-entry catalog and coverage before regenerating the
...
boundary is now 85 paths and its upstream binding count is 66. Its claims, positive
