...
NOTE: Enemy data scraping/emission is disabled. The full enemy table
(thousands of entries, ~30KB inlined into i18n-game.ts and a parallel
array in resources.ts) was costing bundle size for ~20 leyline boss
images that are now mapped explicitly in src/data/resources_manual.ts.
Enemy code paths below are commented out rather than removed so we can
restore them if a future feature actually needs the table.
"""
...
from models import (
    ...
    CharacterSource,
    EffectData,
    # EnemyOutput,  # enemy disabled
    # EnemySource,  # enemy disabled
    HalfSet,
    I18nArtifactData,
    MatchedItem,
    ResourceOutput,
...
from processing import (
    ...
    process_artifacts,
    process_characters,
    # process_enemies,  # enemy disabled
    process_weapons,
)
...
def write_data(
    ...
    details: bool = True,
    # enemy_data: list[EnemyOutput] | None = None,  # enemy disabled
) -> None:
    ...
    _write_resources_ts(
        ...
        weapon_types,
        details=details,
        # enemy_data=enemy_data or [],  # enemy disabled
    )
...
def _serialize_one_per_line(items: Sequence[Any], lean: bool = False) -> str:
    """Serialize a list of models/dicts with each entry on a single line."""
    lines: list[str] = []
    for i in items:
        d = i.model_dump(by_alias=True) if isinstance(i, BaseModel) else dict(i)
        if lean:
            d = {k: v for k, v in d.items() if k in _LEAN_ENTITY_KEYS}
...
def _write_resources_ts(
    ...
    details: bool = True,
    # enemy_data: Sequence[EnemyOutput] = (),  # enemy disabled
) -> None:
    """Write src/data/resources.ts with all entity arrays.

    Each entry is serialized on a single line for compactness.
    ...
    exports: list[tuple[str, str, Sequence[Any], bool]] = [
        ...
        # ("enemies", "EnemyResource[]", enemy_data, False),  # enemy disabled
    ]
    ...
    with open(path, "w", encoding="utf-8") as f:
        ...
        for var_name, ts_type, data, is_entity in exports:
            ...
            f.write(_serialize_one_per_line(data, lean=lean and is_entity))
            f.write(";\n\n")
...
def download_all_images(
    ...
    # enemies: list[MatchedItem[EnemySource]] | None = None,  # enemy disabled
) -> None:
    """Download all character, artifact, element, weapon, and enemy images"""
    script_dir = os.path.dirname(os.path.abspath(__file__))
    project_root = os.path.abspath(os.path.join(script_dir, ".."))
    ...
    #     with tqdm(
    #         enemies,
    #         desc="Downloading Enemies",
    #         unit="img",
    #         bar_format="{l_bar}{bar}| {n_fmt}/{total_fmt}",
    #     ) as pbar:
    #         for match in pbar:
    #             enemy = match["en"]
    #             HoyolabAssetManager.download_enemy_assets(enemy, project_root)
...
def main():
    ...
    # enemy disabled:
    # parser.add_argument("--enemy", action="store_true", help="Update enemy/monster data")
    ...
    # enemy_data = existing_resources.get("enemies", [])  # enemy disabled
    ...
    matched_arts = []
    matched_weaps = []
    # matched_enemies: list[MatchedItem[EnemySource]] = []  # enemy disabled
    new_elements = None
    ...
    # if args.enemy:  # enemy disabled
    #     updated_types.add("enemy")
    ...
    if updated_types or args.half_set:
        write_data(
            ...
            details=args.details,
            # enemy_data=enemy_data,  # enemy disabled
        )
        ...
        download_all_images(
            ...
            new_elements,
            new_weapon_types,
            # enemies=matched_enemies or None,  # enemy disabled
        )
