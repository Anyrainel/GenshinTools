...
- **`baseDmg`**：用于增加固定数值到基础伤害中的效果。例如：“附近的当前场上角色……的伤害提升，提升值相当于<角色名>的X%”，“……造成A类型伤害时，基于<角色名>的B属性，提高造成的伤害”。
...
### 9. 前提假设惯例 (Assumption Conventions)
...
## 第二部分：必备伤害公式
`formulaMap` 必须至少包括这个角色最核心的伤害来源，在不确定时也可以包括其他可能的伤害来源。下面的列表包括了必须涵盖的伤害公式，可以自行添加更多的伤害公式。
...
- 菲谢尔 (fischl): E奥兹攻击伤害
- 菲林斯 (flins): Q初始+2次中间段+最终段伤害（如有水队友则4次中间段），特殊Q雷霆交响（如有水队友则附加额外伤害）
...
- 菈乌玛 (lauma): 【如有水队友】E长按伤害，【C6】普攻1段伤害
