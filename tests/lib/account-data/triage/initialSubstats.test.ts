...
import { startedWithFourSubstats } from "@/lib/account-data/triage/initialSubstats";
...
describe("startedWithFourSubstats", () => {
  it("level 0 with 4 substats is 4-line", () => {
    expect(
      startedWithFourSubstats(
  ...
  it("handles missing substats gracefully", () => {
    expect(startedWithFourSubstats(makeArt({ level: 0 }))).toBe(false);
  });
